...
if st.session_state.state == 'ongoing' and datetime.now().date() <= datetime.strptime(closing_date, '%Y-%m-%d').date():
    ...
    def vote(item):
        ...
        if agegroup =="Till och med gymnasiet":
            ...
            part_ol_traingrp = st.selectbox(f"Ange vilken träningsgrupp i orienteringen du tillhör *",("--Välj grupp--","Rävar","Kaniner","Vit", "Gul", "Orange","Violett", "Vet Ej", "Deltar inte i någon träningsgrupp"))
        else:
            age = None
    ...
    if st.session_state.add_part == True:
        # st.write('OBS! Även den som som står som ansvarig, behöver registreras som deltagare!')
        st.markdown('### **OBS! Även den som som står som ansvarig, behöver registreras som deltagare!**')
